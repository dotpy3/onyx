...
class CasProvider implements AuthenticationProviderInterface
{
	...
	public function __construct(UserProviderInterface $userProvider, $cacheDir, $entityManager, $passwordEncoder, $casClient, $gingerClient)
	{
	...
	public function authenticate(TokenInterface $token)
	{
		...
		try {
			$userLogin = $this->casClient->authenticate($token->getTicket(), $token->getService());
		} catch (Exception $e) {
		...
		try {
			...
			$userInfos = $this->gingerClient->getUser($userLogin);
